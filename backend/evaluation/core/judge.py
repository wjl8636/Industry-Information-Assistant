...
================================================================================
LLM-as-Judge 统一接口（深度研究报告评分）。

对外接口:
    - LLMJudge.score_single(report, query, ground_truth=None) -> dict
    - LLMJudge.compare_two(report_a, report_b, query) -> dict

Judge 维度与项目 DeepResearch 内部 CriticMaster 对齐（事实/逻辑/引用/覆盖），
但使用独立的 EvalLLM 策略，避免"同一条流水线的 Agent 自己给自己打分"的偏差。
================================================================================
"""
...
class LLMJudge:
    """LLM-as-Judge 评审器（基于 DashScope / OpenAI 兼容接口）。"""

    def __init__(self, backend: str = "judge", **llm_kwargs) -> None:
        """backend 传给 EvalLLM 命名；llm_kwargs 可覆盖 model/temperature 等。"""
    ...
    def _get_policy(self):
        if self._policy is None:
            ...
            self._policy = EvalLLM.create(backend=self.backend, **self._llm_kwargs)
        return self._policy
    ...
    def score_single(
        ...
        prompt = f"""你是一位严谨的行业研究报告评审专家。请对以下研究报告进行评分。
...
1. factual_accuracy: 事实准确性（数字、日期、人名、机构名是否正确，有无幻觉）
2. logical_consistency: 逻辑一致性（论证是否自洽，有无矛盾）
3. citation_quality: 引用质量（来源是否可靠，引用是否充分、有据可查）
...
{{
  ...
  "overall": {{"score": 分数, "reason": "简短理由"}}
        ...
        try:
            ...
            logger.warning(f"Judge 单篇评分失败: {e}")
            return {"error": str(e), "judge_backend": self.backend}

        return {"error": "无法解析 Judge 输出", "judge_backend": self.backend}
    ...
    def compare_two(
        ...
        """对两份报告做 head-to-head 对比评分（1-5）。

        返回:
            {
              "comprehensiveness": {"A": 4, "B": 5, "reason": "..."},
              "accuracy": {...}, "structure": {...}, "sources": {...},
              "judge_backend": "judge"
            }
        """
...
{{
  ...
  "sources": {{"A": 分数, "B": 分数, "reason": "简短理由"}}
        ...
        try:
            ...
            logger.warning(f"Judge 对比评分失败: {e}")
            return {"error": str(e), "judge_backend": self.backend}
